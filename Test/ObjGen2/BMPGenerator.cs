...
namespace ObjGen2
{
    public class BMPGenerator
    {
        ...
        public void GenerateBMP(FieldPlan fieldPlan, string outputFolder)
        {
            if (fieldPlan?.Bins?.BinTable?.Bins == null || fieldPlan.Bins.BinTable.Bins.Count == 0)
            {
                throw new InvalidOperationException("No bin data available for BMP generation");
            }
            ...
            var bmpData = CreateBMPData(elevationGrid, gridWidth, gridHeight, scaledWidth, scaledHeight, 
                minElevation, maxElevation, colorPalette);

            // Write BMP file
            var fileName = Path.Combine(outputFolder, "elevation_grid.bmp");
